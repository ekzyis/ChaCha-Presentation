...
\end{minipage}
\end{figure}
Lösung: Jeden Übergang einzeln implementieren \vspace{0.5em}\\
Problem:\\
\hspace{1em}%
\begin{minipage}[c]{0.95\textwidth}
\flushleft Sehr aufwändig, da für jede Kombination an Seitenzuständen benötigt
\end{minipage}
\end{frame}

% Move problem to own frame with graph showing that amount of transitions increases with O(n!) (include plots of O(n) and O(2^n) as comparison?)
...
$\Rightarrow$ Pro neuer Seitenzustand nur zwei neue Übergänge benötigt \vspace{0.5em}\\
Problem: Performance (skaliert mit $\mathcal{O}(n)$)\\
\hspace{1em} (Reaktionszeit von bis zu 40\,s (!))
\end{frame}

% Move problem to own frame with performance graph from thesis paper
...
$\Rightarrow$ Reaktionszeit verringert auf maximal 1.5\,s \vspace{0.5em}\\
Problem: Performance inkonsistent und intransparent \\
\hspace{1em}(variiert zwischen 5\,ms und 1.5\,s; unabhängig von Übergängsgröße)
\end{frame}

% Move problem to own frame with performance graphs from thesis paper
